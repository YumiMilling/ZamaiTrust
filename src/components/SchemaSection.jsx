...
export default function SchemaSection() {
  return (
    <section className="sec">
      <div className="eye">The Digital Receipt</div>
      <h2 className="h2">Every transaction gets the same envelope.</h2>
      <p className="p">Think of it like a standardised form at the post office. Whether you're recording a grain delivery, a quality check, or a payment — it all goes into the same kind of envelope. This makes every record easy to find, easy to verify, and impossible to fake.</p>
      <div className="code-block">
        <div className="code-label">What's inside the envelope</div>
        <pre>{`{
  ...
  `}<span className="ck">"payload"</span>{`:         { `}<span className="cc">/* details specific to this type */</span>{` }
}`}</pre>
      </div>
      <div className="note">
        <div className="note-title">Two Signatures, Always</div>
        <div className="note-body">Every record needs two people to sign — like a witness at a ceremony. The farmer and the depot both confirm the delivery. If they disagree, both versions are kept on file. Disagreements are evidence, not errors.</div>
      </div>
